...
const STORY_URLS = [
  ...
  async function getStories() {
    ...
    let storyLocalStorage = localStorage.getItem("story");
    if (storyLocalStorage) {
      return JSON.parse(storyLocalStorage);
    }
  
    const stories = [];
  
    return new Promise(async (resolve, reject) => {
      const getPromises = RECIPE_URLS.map(async (url) => {
        try {
          const siteResponse = await fetch(url);
          const storyToAdd = await siteResponse.json();
          stories.push(recipeToAdd);
        } 
        catch (err) {
          console.error(err);
        }
   ...
   */
  function saveStoriesToStorage(recipes) {
   ...
   * @param {Array<Object>} recipes An array of recipes
   */
  function addStoriesToDocument(stories) {
    if (!stories) return;
